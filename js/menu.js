function createMenuButton() {
	...
	// </div>
	$('#container').append($('<a>',{href:'javascript:void(0)',id:'menu-toggle-wrapper'}));
	$('#container').append($('<div>',{style:'opacity:.9;',id:'menu'}));

	$('#menu-toggle-wrapper').append($('<div>',{id:'menu-toggle'}));
	$('#menu').append($('<img>',{id:'menu_img', src:'images/back.jpg'}));
...
function createSevereTables() {
	...
	$('#container').append($('<img>',{id:'severe-table',src:''}));
...
function addLocationTable(location, top) {
	$('#container').append($('<img>',{class:location,id:'severe',src:'images/icons/'+location+'.png', style:'top:'+top+';'}))
	$('#container').append($('<img>',{class:location,id:'severe-table',src:'images/severe injuries/'+location+'.png'}))
...
function createReference() {
	...
	$('#container').append($('<img>',{id:'reference',src:'images/icons/reference.png'}));
	$('#reference').hide();
	$('#reference').delay(1000).fadeIn(2000);

	$('#container').append($('<div>',{id:'reference-window-back0'}));
	$('#reference-window-back0').append($('<img>',{id:'reference-window-back-img', src:'images/reference/reference_back.png'}));
	...
	$('#reference-window-back').append($('<img>',{id:'glossary-symbols',src:'images/reference/symbols.png'}));
	...
	var selectize = $('#reference-window').selectize({
		options: all_option,
		optgroups: [
				{value: 'random draws', label: 'Random Draws'},
				{value: 'settlement events', label: 'Settlement Events'},
				{value: 'glossary', label: 'Glossary'},
				{value: 'abilities', label: 'Abilities'},
				{value: 'fightning arts', label: 'Fightning Arts'},
				{value: 'disorders', label: 'Disorders'},
				{value: 'secret fightning arts', label: 'Secret Fightning Arts'},
		...
		onItemRemove: function(values) {
				...
				setTimeout(function(){$('#reference-data.'+adapt_name(values)).remove()}, 1500);
		...
		onItemAdd: function(values, item) {
				...
				if (is_random_draw(values)) {
					...
					for (let i = 0; i < draws.length; i++) {
						console.log(draws[i])
						setTimeout(function(){selectize.addItem(draws[i], false);$('#glossary-symbols').hide();}, 500*i);
					}
