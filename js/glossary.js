...
function get_options(data, type) {
	...
	for (let key in data) {
		// console.log(key)
		result.push({class:type, name:key, value:key})
	}
...
function get_random_draws(word) {
	...
	if (word.includes('Fightning')) {
		return getRandom(Object.keys(fightning_arts_text), number)
	} else if (word.includes('Disorder')) {
		return getRandom(Object.keys(disorders_text), number)
	} else if (word.includes('Settlement Event')) {
		let list = Object.keys(settlement_events)
		list = removeA(list, 'First Day');
		return getRandom(list, number)
	} else {
		return [];
	}
}
